...
| 渠道 | 状态 | 设置 |
|------|------|------|
| macOS 桌面通知 | 默认启用 | 确保 **系统设置 → 通知 → Script Editor** 已开启 |
| Telegram | 需要配置 | 见下方 |
...
**默认已启用。** 通过 `osascript` 发送 macOS 原生通知。
...
> **注意：** 通知通过 Script Editor 投递。请确保在 **系统设置 → 通知 → Script Editor** 中开启了通知权限。如果收不到通知，请先检查此项设置。
