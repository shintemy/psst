...
<p align="center">
  <em>Every token counts, it counts for you.</em><br/>
  嘘，悄悄提醒你一下 —— 你的 AI 额度快用完了。
</p>

<p align="center">
  <strong>Runs entirely on your machine. No data leaves your computer.</strong><br/>
...
Psst 是一个本地常驻服务，监控你所有 AI 编码工具的用量配额。当额度快用完或即将重置时，通过多渠道推送提醒，帮你避免额度浪费。
...
启用的渠道: Desktop, Telegram, Web Push
...
4. In the JSON response, find the `"chat"` object and note the `"id"` number. For example:
   ...
   ```
   In this example, the chat ID is `1522808475`.
...
You should receive a message from your bot in Telegram saying "Psst! 测试通知".
...
| Cursor | 本地 SQLite (`~/.cursor/ai-tracking/`) |
...
如果配置正确，你会在 Telegram 中收到 Bot 发来的 "Psst! 测试通知" 消息。
...
8. 点击页面上的 **"Enable Push Notifications"** 按钮，在弹出的权限请求中点击**"允许"**
