...
Psst 是一个开源的本地常驻服务，监控 Claude 和 Cursor 的用量配额，在关键节点通过多渠道推送提醒用户，避免额度浪费或超限。
...
使用 Claude（Pro/Max）和 Cursor（Pro/Max）的开发者，希望：
- 随时了解剩余用量
- 在额度快用完时收到提醒
- 在重置窗口前收到提醒，避免额度清零浪费
...
#### 1.3 Claude 历史消耗（补充数据）

**方式：** tokscale-core 解析 `~/.claude/projects/**/*.jsonl`

- 提供详细的 token 消耗历史（用于仪表盘展示）
- 与 OAuth API 的配额百分比互补
...
│   ├── data_sources/
│   │   ├── mod.rs
│   │   ├── claude_quota.rs          # Claude OAuth API 配额查询
│   │   ├── claude_usage.rs          # tokscale-core 解析 Claude 消耗
│   │   └── cursor_usage.rs          # tokscale-core 解析 Cursor 消耗 + 配额推算
│   ├── threshold.rs                 # 阈值引擎
│   ├── notifiers/
